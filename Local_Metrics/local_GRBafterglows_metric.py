...
# --------------------------------------------
def sample_grb_rate_from_volume(t_start, t_end, d_min, d_max, rate_density=1e-5): #1e-8 to account for dirty fireball and off axis, 1e-9 without
    """
...
def generateGRBPopSlicer(t_start=1, t_end=3652, seed=42,
                         d_min=10, d_max=1000, num_lightcurves=1000, gal_lat_cut=None,
    ...
    rng = np.random.default_rng(seed)
    n_events = sample_grb_rate_from_volume(t_start, t_end, d_min, d_max)
